...
def mask_account_card(account_number):
    ...
    if len(parts) > 1:
        # Если номер карты состоит из нескольких частей
        # (например, Visa Gold 5999 4142 2842 6353)
        masked_parts = []
        for part in parts[1:]:
            if len(part) <= 4:
                masked_part = part
        ...
        # Если номер карты представлен одной строкой
        # (например, MasterCard 1234567890123456)
